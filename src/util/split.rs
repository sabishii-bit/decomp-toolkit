...
fn split_extabindex(obj: &mut ObjInfo, start: SectionAddress) -> Result<()> {
    log::info!("split_extabindex called for section {} @ {:#X}", start.section, start.address);
    ...
    // Collect extabindex entries to determine link order
    let mut extab_entries: Vec<(SectionAddress, SectionAddress)> = Vec::new();
    ...
    while current_address.address < section_end {
        ...
        // Collect this entry for link order determination
        extab_entries.push((function_addr, extab_addr));
    ...
    // Populate link_order based on extab addresses to preserve original link order.
    // The linker concatenates extab sections in link order, so sorting by extab address
    // gives us the exact original link order even when function addresses don't match.
    //
    // However, this only works if most/all objects have extab entries. If extab coverage
    // is low (<80%), we can't reliably determine link order from extab alone, so we skip
    // this optimization and rely on address-based ordering instead.

    // Count total functions in code sections (not obj.link_order which may be incomplete)
    let mut total_functions = 0;
    for (_, section) in obj.sections.iter() {
        if section.kind == ObjSectionKind::Code {
            total_functions += section.splits.iter().count();
        }
    }

    let extab_units = extab_entries.len();
    let coverage_percent = if total_functions > 0 { (extab_units * 100) / total_functions } else { 0 };

    log::info!("Extab coverage: {} extab entries / {} code functions = {}%",
        extab_units, total_functions, coverage_percent);

    if coverage_percent >= 80 {
        let mut sorted_entries = extab_entries.clone();
        sorted_entries.sort_by_key(|(_, extab_addr)| (extab_addr.section, extab_addr.address));

        for (order_idx, (function_addr, _)) in sorted_entries.iter().enumerate() {
            // Find the unit name for this function
            if let Some((_, text_section)) = obj.sections.at_address(function_addr.address).ok() {
                if let Some((_, split)) = text_section.splits.for_address(function_addr.address) {
                    let unit_name = &split.unit;
                    // Check if this unit already exists in link_order
                    if let Some(existing_unit) = obj.link_order.iter_mut().find(|u| &u.name == unit_name) {
                        // Update the order if not already set (prefer the first/earliest extab order)
                        if existing_unit.order.is_none() {
                            existing_unit.order = Some(order_idx as i32);
                        }
                    } else {
                        // Add new unit with order
                        obj.link_order.push(ObjUnit {
                            name: unit_name.clone(),
                            autogenerated: obj.is_unit_autogenerated(unit_name),
                            comment_version: None,
                            order: Some(order_idx as i32),
                        });
                    }
                }
            }
        }
        log::info!("Set link order for {} units based on extab addresses ({}% coverage)",
            obj.link_order.iter().filter(|u| u.order.is_some()).count(), coverage_percent);
    } else {
        log::info!("Skipping extab-based link order (only {}% coverage, need >=80%)", coverage_percent);
    }
...
pub fn update_splits(obj: &mut ObjInfo, common_start: Option<u32>, fill_gaps: bool) -> Result<()> {
    ...
    // Extract link order from .ctors section
    // The .ctors section contains constructor pointers in link order, providing
    // a reliable source of link order information even when extab coverage is low.
    if let Some((_section_index, ctors_section)) = obj.sections.by_name(".ctors")? {
            let splits_count = ctors_section.splits.iter().count();
            log::info!(".ctors section found with {} bytes, {} splits",
                ctors_section.data.len(), splits_count);
            if !ctors_section.data.is_empty() && splits_count > 0 {
                // Start order index after extab entries (if any)
                let mut order_idx = obj.link_order.iter()
                    .filter_map(|u| u.order)
                    .max()
                    .map(|max| max + 1)
                    .unwrap_or(0);
                let mut ctors_units_ordered = Vec::new();
                let mut units_set = 0;

                // Iterate through .ctors splits in address order (which is link order for .ctors)
                for (_addr, split) in ctors_section.splits.iter() {
                    let unit_name = &split.unit;
                    if !ctors_units_ordered.contains(&unit_name.as_str()) {
                        ctors_units_ordered.push(unit_name.as_str());
                    }
                }

                log::info!("Found {} unique units in .ctors section", ctors_units_ordered.len());

                // Set order values for units that have .ctors entries
                // IMPORTANT: Extab order takes precedence over .ctors order!
                // Units with both extab and .ctors should use extab order to preserve extab section layout.
                // The .ctors section is collected by the linker in link order, so it will still work correctly.
                for unit_name in &ctors_units_ordered {
                    if let Some(existing_unit) = obj.link_order.iter_mut().find(|u| &u.name == unit_name) {
                        // Only set .ctors order if unit doesn't already have extab order
                        if existing_unit.order.is_none() {
                            existing_unit.order = Some(order_idx);
                            order_idx += 1;
                            units_set += 1;
                        }
                        // If unit already has extab order, skip it (extab takes precedence)
                    } else {
                        // Add new unit with .ctors-based order
                        obj.link_order.push(ObjUnit {
                            name: unit_name.to_string(),
                            autogenerated: obj.is_unit_autogenerated(unit_name),
                            comment_version: None,
                            order: Some(order_idx),
                        });
                        order_idx += 1;
                        units_set += 1;
                    }
                ...
                if units_set > 0 {
                    log::info!("Set link order for {} additional units based on .ctors section", units_set);
                }
            }
    } else {
        log::info!("No .ctors section found");
    }
...
fn resolve_link_order(obj: &ObjInfo) -> Result<Vec<ObjUnit>> {
    eprintln!("DEBUG: resolve_link_order() called!");
    ...
    // Build a map of unit name to its address in the first code section (.init or .text)
    // We prioritize code sections because that determines the final function addresses
    let mut unit_addr = BTreeMap::<&str, u64>::new();
    for (_, section, addr, split) in obj.sections.all_splits() {
        // Only use code sections for determining link order
        if section.kind == ObjSectionKind::Code {
            let abs_addr = section.address + addr as u64;
            unit_addr.entry(split.unit.as_str()).or_insert(abs_addr);
        }
    }

    // For units without code sections (data-only), use their first appearance
    for (_, section, addr, split) in obj.sections.all_splits() {
        if !unit_addr.contains_key(split.unit.as_str()) {
            let abs_addr = section.address + addr as u64;
            unit_addr.entry(split.unit.as_str()).or_insert(abs_addr);
        }
    }

    // Sort units by their code section address to get the correct link order
    let mut unit_addr_pairs: Vec<_> = unit_addr.into_iter().collect();
    unit_addr_pairs.sort_by_key(|(_, addr)| *addr);

    // For matching decompilation, we use address-based ordering directly
    eprintln!("DEBUG: Using address-based link order for {} units", unit_addr_pairs.len());

    // Log cyclic units for debugging
    let cyclic = ["800DA60C", "800DA6A4", "800DA920", "800DACD8", "800DB008", "800DB0F8", "800DB15C", "800DB1BC"];
    eprintln!("DEBUG: Cyclic units in address order:");
    for (unit, addr) in &unit_addr_pairs {
        if cyclic.iter().any(|c| unit.contains(c)) {
            eprintln!("DEBUG:   {} at {:#010X}", unit, addr);
        }
    }

    let mut link_order: Vec<ObjUnit> = unit_addr_pairs
        .into_iter()
        .map(|(unit, _addr)| {
            if let Some(existing) = obj.link_order.iter().find(|u| u.name == unit) {
                existing.clone()
            } else {
                ObjUnit {
                    name: unit.to_string(),
    ...
    // If any units have explicit order values, use those to re-sort the link order
    // This allows preserving the original link order from the binary even when
    // function addresses don't match link order
    if link_order.iter().any(|u| u.order.is_some()) {
        eprintln!("DEBUG: Using explicit order values for {} units", link_order.iter().filter(|u| u.order.is_some()).count());

        // Debug: Show first 20 units before sorting
        eprintln!("DEBUG: First 20 units BEFORE sorting by order:");
        for (i, u) in link_order.iter().take(20).enumerate() {
            eprintln!("DEBUG:   [{}] {} (order: {:?})", i, u.name, u.order);
        }

        // Sort by order, but treat None as i32::MAX so units without order come last
        link_order.sort_by_key(|u| (u.order.unwrap_or(i32::MAX), u.name.clone()));

        // Debug: Show first 20 units after sorting
        eprintln!("DEBUG: First 20 units AFTER sorting by order:");
        for (i, u) in link_order.iter().take(20).enumerate() {
            eprintln!("DEBUG:   [{}] {} (order: {:?})", i, u.name, u.order);
        }

        // Debug: Show units 508-520 (around extab/.ctors boundary)
        eprintln!("DEBUG: Units 508-520 (extab/.ctors boundary):");
        for (i, u) in link_order.iter().skip(508).take(13).enumerate() {
            eprintln!("DEBUG:   [{}] {} (order: {:?})", i+508, u.name, u.order);
        }
    }
...
pub fn split_obj(
    ...
    for (section_index, section) in obj.sections.iter() {
        ...
        loop {
            ...
            if current_address.address & (align as u32 - 1) != 0 {
                ...
                current_address.address & (align as u32 - 1) == 0,
